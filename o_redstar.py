...
class Rs:
    ...
    def init(bot_ref):
        ...
        Rs.qm4 = QueueManager('RS4', 4, 0xff3300,
                              discord.utils.get(bot.get_guild(params.SERVER_DISCORD_ID).roles,
                                                name=params.RS4_ROLE).mention)
        Rs.qm5 = QueueManager('RS5', 5, 0xff3300,
                              discord.utils.get(bot.get_guild(params.SERVER_DISCORD_ID).roles,
                                                name=params.RS5_ROLE).mention)
        Rs.qm6 = QueueManager('RS6', 6, 0xff3300,
                              discord.utils.get(bot.get_guild(params.SERVER_DISCORD_ID).roles,
                                                name=params.RS6_ROLE).mention)
        Rs.qm7 = QueueManager('RS7', 7, 0xff3300,
                              discord.utils.get(bot.get_guild(params.SERVER_DISCORD_ID).roles,
                                                name=params.RS7_ROLE).mention)
        Rs.qm8 = QueueManager('RS8', 8, 0xff3300,
                              discord.utils.get(bot.get_guild(params.SERVER_DISCORD_ID).roles,
                                                name=params.RS8_ROLE).mention)
        Rs.qm9 = QueueManager('RS9', 9, 0xff3300,
                              discord.utils.get(bot.get_guild(params.SERVER_DISCORD_ID).roles,
                                                name=params.RS9_ROLE).mention)
        Rs.qm10 = QueueManager('RS10', 10, 0xff3300,
                               discord.utils.get(bot.get_guild(params.SERVER_DISCORD_ID).roles,
                                                 name=params.RS10_ROLE).mention)
        Rs.qm11 = QueueManager('RS11', 11, 0xff3300,
                               discord.utils.get(bot.get_guild(params.SERVER_DISCORD_ID).roles,
    ...
    async def enter_queue(caller: discord.Member,
        ...
        if res == QueueManager.PLAYER_JOINED:
            m = await bot.get_channel(params.SERVER_RS_CHANNEL_ID).send(f':new: {player.discord_nick} joined {ping_string} ({queue_len}/4).')
        ...
        if caused_by_afk is True:
            ...
            for qm in Rs.qms:
                ...
                if res == QueueManager.PLAYER_LEFT:
                    ...
                    m = await bot.get_channel(params.SERVER_RS_CHANNEL_ID).send(f'{params.LEAVE_EMOJI} {player.discord_nick} timed out for {qm.name} ({len(q)}/4)')
                    await m.delete(delay=params.MSG_DISPLAY_TIME)
